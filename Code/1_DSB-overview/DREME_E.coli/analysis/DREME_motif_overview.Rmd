...
for (s in names(all_seqlogos)) {
    ...
    all_grobs = append(all_grobs, tmp )
}
...
<!-- ```{r} -->
<!-- pdf(file="DREME_all_motifs.pdf", width = 11, height = 14) -->
<!-- print(p) -->
<!-- dev.off() -->
<!-- ``` -->
...
all_grobs = list()
for (s in names(all_seqlogos)) {
    tmp = all_seqlogos[[s]][1:3]
    if(length(tmp)<3) {for(i in 1:(10-length(tmp))) tmp[[paste0(s,"_t",i)]] = empty }
    all_grobs = append(all_grobs, tmp )
}
p = marrangeGrob(all_grobs, nrow=3, ncol=4, top="" )  # layout_matrix = matrix(c(1,2,2,2,3,3,4,4), ncol=2, nrow=4)
print(p)  
```

```{r}
pdf(file="../../../../Results/1_DSB-overview/DREME_all_motifs_pks+_vs_pks-_E.coli_top3.pdf", width = 11, height = 4)
print(p)
dev.off()
```
