...
```{r}
nn_means = lr_shape[central2_short3=="SS", .(values=list(Mean)), by=c("backbone_class_4a5")]
nn_means[, backbone_class_4a5:=as.character(backbone_class_4a5)]
setkey(nn_means, "backbone_class_4a5")

bb = unique(as.character(lr_shape$backbone_class_4a5))

test_fun = function(b, n) wilcox.test(subset(lr_shape, backbone_class_4a5==b & central2_short3==n)$Mean, unlist(nn_means[b]$values))
...
test_res = data.frame(central2_short3 = nuc_n, backbone_class_4a5 = factor(backbone_n, levels=levels(lr_shape$backbone_class_4a5)), test_pval = unlist(lapply(aa, `[` , "p.value") ), stringsAsFactors = F)
...
t_no_y = theme_bw() + theme(text=element_text(size=22), legend.position = "none" , axis.title.y=element_blank(),  axis.text.y=element_blank(), axis.ticks.y=element_blank())
t_y = theme_bw() + theme(text=element_text(size=22), legend.position = "none" )

p1 = ggplot(lr_shape, aes(y=Mean, x=MGW.avgV3V4)) + geom_point(aes(col=central2_short3), size=2) + geom_smooth() + facet_grid(backbone_class_4a5 ~ .) + geom_hline(yintercept = 0, col="red") + ylab("Mean Log2 Ratio") + xlab("Avg. MGW N3,N4") + t_y + coord_cartesian(ylim=c(-0.1,1.1)) + theme(strip.text.y=element_blank())

p1b = ggplot(lr_shape, aes(y=Mean, x=EP.avgV3V4)) + geom_point(aes(col=central2_short3), size=2) + geom_smooth() + facet_grid(backbone_class_4a5 ~ .) + geom_hline(yintercept = 0, col="red") + ylab("Mean Log2 Ratio") + xlab("Avg. EP N3,N4") + t_no_y + coord_cartesian(ylim=c(-0.1,1.1)) + theme(strip.text.y=element_blank())

p1c = ggplot(lr_shape, aes(y=Mean, x=Stiffness.avgV2V3V4)) + geom_point(aes(col=central2_short3), size=2) + geom_smooth() + facet_grid(backbone_class_4a5 ~ .) + geom_hline(yintercept = 0, col="red") + ylab("Mean Log2 Ratio") + xlab("Avg. Stiffness S2,S3,S4") + t_no_y + coord_cartesian(ylim=c(-0.1,1.1)) + theme(strip.text.y=element_blank())


p2 = ggplot(lr_shape, aes(y=Mean, x=central2_short3)) + geom_boxplot(aes(fill=central2_short3)) + facet_grid(backbone_class_4a5 ~ .) + geom_hline(yintercept = 0, col="red") + geom_text(data=test_res, aes(label=sig_label, y=1, x=central2_short3), inherit.aes = FALSE, col="red", size=10) + coord_cartesian(ylim=c(-0.1,1.1)) + ylab("Mean Log2 Ratio") + t_no_y + xlab("central 2 nucleotides")
...
```{r}
pdf("../../../Results/3_DSB-shape-characteristics/Pattern_shape_model.pdf", width=16, height = 14)
marrangeGrob(list(p1,p1b,p1c, p2), ncol=4, nrow=1, top = "" )
dev.off()
```
