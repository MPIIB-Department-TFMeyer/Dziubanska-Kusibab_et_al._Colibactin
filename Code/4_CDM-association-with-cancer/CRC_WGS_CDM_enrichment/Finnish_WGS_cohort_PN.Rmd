---
title: "Finnish WGS cohort - PN counts"
...
```{r}
data = read.table("../../../Data/WGS_cohort/Mutation_counts_at_motifs/PN_all_mut_counts_CRC_WGS_n200.txt", sep="\t", header=T, comment.char = "")
...
colnames(data)[2:ncol(data)] = ed[colnames(data)[2:ncol(data)], "ID"]
```

```{r}
all_W5_pattern = data.frame(pattern = as.character(mkAllStrings(c("A","T"), 5)), stringsAsFactors = F)
all_W5_pattern$a_cnt = unlist(Map(function(x) sum(alphabetFrequency(DNAString(x))[c("A")]), all_W5_pattern$pattern))
all_W5_pattern$revcomp = as.character(reverseComplement(DNAStringSet(all_W5_pattern$pattern)))
all_W5_pattern = all_W5_pattern[order(all_W5_pattern$a_cnt, decreasing = T),]
all_W5_pattern$index = 1:nrow(all_W5_pattern)
all_W5_pattern$rc_index = match(all_W5_pattern$pattern, all_W5_pattern$revcomp)
all_W5_pattern_norevcomp = subset(all_W5_pattern, rc_index >= index)
```


```{r}
load("../../../Data/Pattern_frequencies_and_positions/PN_pos_genomic_n100_and_AT6_hg38.Rdata")
total_target_size = sum(as.numeric(width(all_standard_chrome_ranges)))
```

# Overview over mutation frequency at WWWWW patterns

```{r, fig.height=10, fig.width=16}
tmp = data[data$X.motif %in% all_W5_pattern_norevcomp$pattern,]
mm = as.matrix(tmp[, 2:ncol(tmp)])
rownames(mm) = tmp$X.motif
col_anno = ed[, "group", drop=F]
rownames(col_anno) = ed$ID
row_anno = all_W5_pattern_norevcomp[, "a_cnt",drop=F]
rownames(row_anno)   = all_W5_pattern_norevcomp$pattern
#png(file="WWWWWW_motif_freq_HM.png", width=1800, height=900)
pheatmap(mm, scale="column", annotation_col = col_anno, show_colnames = F, fontsize_row = 10, main = "WWWWW counts, scaled per sample", annotation_row = row_anno)
#dev.off()
```
...
# Mutation rates per bp in genome
...
rownames(col_anno) = ed$ID
#png(file="WWWWWW_motif_freq_HM.png", width=1800, height=900)
pheatmap(tmp2, scale="column", annotation_col = col_anno, show_colnames = F, fontsize_row = 10, main = "WWWWW motif mutation rates (mutations/bp covered), scaled per sample")
```

```{r}
analysis_name = "Finnish_COREAD_WGS_cohort"
ofile = paste0("../../../Data/WGS_cohort/5nucleotide_Results_", analysis_name ,"_all_mutations.Rdata")
save(combined_tab, all_W5_pattern_norevcomp, file=ofile)
```
