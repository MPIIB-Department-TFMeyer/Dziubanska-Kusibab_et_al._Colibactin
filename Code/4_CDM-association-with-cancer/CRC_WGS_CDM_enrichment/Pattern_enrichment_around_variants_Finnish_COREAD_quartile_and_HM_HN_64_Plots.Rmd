---
title: "Pattern overlap global - Finnish COREAD data, single_samples, WWWWWW hexanucleotides - Plots"
...
tt = regmatches(ed$X.Sample, regexpr("(.+)_T_", ed$X.Sample))
ed$ID = make.names(substr(tt, 1, nchar(tt)-3))
rownames(ed) = make.names(ed$X.Sample)

tt2 = sapply(strsplit(ed$X.Sample,"-"), `[`,1 )
ed$group = ifelse(tt2 %in% c("POLE","MSI"), tt2, "other" )
```

# Overview

Results in this document describe the analysis of enrichment at the hexanucleotide motif AAWWTT in a data set of 200 colorectal cancer samples from a finnisch CRC cohort with WGS sequencing.

Mutations were analyzed for SNV at A/T reference bases.

Mutation rates (mutations per bp of the genomic target sequence covered by a given hexanucleotide pattern) were determined for all possible 64 WWWWWW hexanucleotides (36 after removal of reverse complement duplicates). The mutation rate of AAWWTT was compared to the distribution of mutation rates for hexanucleotides consisting only of A/T (furtheron referred to as [A/T]6).

```{r}
analysis_name = "Finnish_COREAD_WGS_cohort"
all_data_files = list.files(path="../../../Data/WGS_cohort", pattern=paste0("6nucleotide_Results_",analysis_name,".+Rdata"), full.names = T)
...
for (n in names(all_data_files)) {
  ...
  combined_tab$cancer_entity = factor(ed[as.character(combined_tab$entity),"group"], levels=c("MSI","POLE","other"))
  combined_tab$count_class = combined_tab$entity

  combined_tab = subset(combined_tab, pattern %in% all_W6_pattern_norevcomp$pattern)
...
n = mut_sets_ordered[1]
  ...
  p_at6 = mapply(function(log2r, class) {a = log2r[class=="AAWWTT"]; b = log2r[class=="[A/T]6"]; if(length(a)>1 && length(b)>1) {p=wilcox.test(a,b )$p.value} else {p=NA}; return(p)}, l2r, cl)
 ...
 at_rich_non_sel_stats = tmp[tmp$at_rich_non_sel, .(vmean=mean(pn_vs_at6), vsd=mad(pn_vs_at6), n=.N, t97.5=qt(0.975, .N-1), entity = unique(entity)), by=c("cancer_entity", "count_class")]
 ...
 tmp$sig_label = with(tmp, ifelse(pn_vs_at6>0 & p_at6 < 0.05 & padj_at6 < 0.2, "*", ""))
...
setnames(tmp3, "pn_vs_at6", "log2ratio_adjusted")
...
x2 = unique(tmp3[, c("entity","total_mutation_counts", "p_at6", "padj_at6", "cancer_entity"), with=F])
setkey(x2, "entity")

per_sample_table = merge(xx, x2)
per_sample_table[, mean_log2r_colibactin_HN:= (log2ratio_adjusted_AAAATT+log2ratio_adjusted_AAATTT)/2]
per_sample_table[, total_mut_count_colibactin_HN:= mutation_hit_AAAATT + mutation_hit_AAATTT]
per_sample_table[, total_mut_proportion_colibactin_HN:= total_mut_count_colibactin_HN/total_mutation_counts]
ed2 = ed
rownames(ed2) = ed2$ID
per_sample_table[, sample_name_full := ed2[as.character(entity),]$X.Sample ]
fwrite(per_sample_table, file="../../../Data/WGS_cohort/Per_sample_results_HN_SNV_enrichment.txt", sep="\t", quote = F)
```
...
rects <- data.frame(xstart = c(0,ce_counts[1:2])+0.5, xend = ce_counts+0.5, 
         col = names(ce_counts))
p1 = ggplot(sel_tmp, aes(y=pn_vs_at6, x=entity)) + geom_point(aes(col=pattern)) +
...
p3 = ggplot(sel_tmp, aes(y=(mutation_hit+1)/total_mutation_counts, x=entity)) + geom_point(aes(col=pattern)) +
        ...
        coord_cartesian(ylim=c(1e-3,0.05)) + 
...
```{r, fig.width=18, fig.height=9}
png(file="../../../Results/4_CDM-association-with-cancer/Finnish_cohort_COREAD_enrichment_HN_SNV.png", width = 1800, height=900)
ggarrange(p1,p2,p3, ncol=1, align="v", heights = c(0.5, 0.25, 0.25), common.legend = T)
dev.off()
```

```{r, fig.width=18, fig.height=9}
cairo_pdf(file="../../../Results/4_CDM-association-with-cancer/Finnish_cohort_COREAD_enrichment_HN_SNV_cairo.pdf", width = 18, height=9) 
ggarrange(p1,p2,p3, ncol=1, align="v", heights = c(0.5, 0.25, 0.25), common.legend = T)
dev.off()
```
