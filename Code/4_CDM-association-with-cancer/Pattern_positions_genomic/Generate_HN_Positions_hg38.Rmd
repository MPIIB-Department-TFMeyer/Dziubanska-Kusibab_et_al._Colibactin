...
We want to analyze possible enrichment of mutations at individual hexanuclear patterns in cancers.  We identify hexanucleotide positions genomewide for selected HN, all HN with 6 A/T and 100 random PN. 
...
of = oligonucleotideFrequency(g_views, 6)
oligofreqs_genomic = apply(of, 2, sum)
```


```{r}
# this includes AAWWTT and all other [A/T]6 hexanucleotides
pattern_dict_sel = DNAStringSet(mkAllStrings(c("A","T"), 6))

set.seed(12345)
pattern_dict_r = DNAStringSet(mkAllStrings(c("A","C","G","T"), 6))[sample.int(1024, 100)]
...
```{r}
curr_date = Sys.time()
save(pattern_pos, pattern_dict, pattern_bp_covered, oligofreqs_genomic, all_standard_chrome_ranges, curr_date, file="HN_pos_genomic_n100_and_AT6_hg38.Rdata")
```
