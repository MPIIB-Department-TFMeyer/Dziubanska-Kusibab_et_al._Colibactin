...
```{r}
pattern_dict = DNAStringSet(c("AAATTT","AAAATT","AATATT", "TAAAAA","CAAAAA", "AAAAAG", "ATATAG", "AACGTT", "AAGCTT","TTCGTA" ,"TTGGTA","TTCTTA","CGCGCT", "TTTAAA"))
...
sel_conditions = c("EcoliMut", "EcoliWT", "Ctrl")
...
final_tab_agg = final_tab_agg[, CI_prop_dir:=qt(0.975, rep_cnt)*(sd_prop_dir/sqrt(rep_cnt))]
...
p = ggplot(final_tab_agg, aes(x=dist, y=mean_prop_dir, fill=strand)) + geom_bar(stat = "identity", width = 0.8) + 
  ...
  #geom_text(data=text_labels, aes(x=x, y=y, label=t), inherit.aes = F, size=4, col="black") +
  #geom_text(data=text_lines, aes(x=x, y=y, label=t), inherit.aes = F, size=4, col="blue1") + 
  theme_few() +
  theme(text = element_text(size=16), strip.text.x = element_text(size = 16), strip.text.y = element_text(size = 20)) + 
...
final_tab_agg = final_tab_agg[, CI_diff:=qt(0.975, rep_cnt)*(sd_diff/sqrt(rep_cnt))]
...
p = ggplot(subset(final_tab_agg, mean_diff*ifelse(strand=="f",1,-1) > 0), aes(x=dist, y=mean_diff, fill=strand)) + 
  ...
  #geom_text(data=text_lines, aes(x=x, y=y, label=t), inherit.aes = F, size=4, col="blue1") + 
  theme_few() + 
  theme(text = element_text(size=12), strip.text.x = element_text(size = 16), strip.text.y = element_text(size = 20)) + 
